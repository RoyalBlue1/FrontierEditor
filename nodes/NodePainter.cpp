...
void NodePainter::drawNodeRect(
	QPainter* painter, NodeGeometry const& geom, NodeDataModel const* model, NodeGraphicsObject const& graphicsObject)
{
...
void NodePainter::drawConnectionPoints(
	QPainter* painter, NodeGeometry const& geom, NodeState const& state, NodeDataModel const* model, FlowScene const& scene)
{
	NodeStyle const& nodeStyle = model->nodeStyle();
	...
	for (PortType portType : {PortType::Out, PortType::In})
	{
		...
		for (unsigned int i = 0; i < n; ++i)
		{
			...
			auto const& dataType = model->dataType(portType, static_cast<int>(i));

			bool canConnect =
				(state.getEntries(portType)[i].empty() ||
				 (portType == PortType::Out && model->portOutConnectionPolicy(i) == NodeDataModel::ConnectionPolicy::Many));
...
void NodePainter::drawFilledConnectionPoints(
	QPainter* painter, NodeGeometry const& geom, NodeState const& state, NodeDataModel const* model)
{
	NodeStyle const& nodeStyle = model->nodeStyle();
	auto const& connectionStyle = StyleCollection::connectionStyle();
	...
	for (PortType portType : {PortType::Out, PortType::In})
	{
		...
		{
			QPointF p = geom.portScenePosition(static_cast<PortIndex>(i), static_cast<PortType>(portType));
			...
			if (!state.getEntries(portType)[i].empty())
			{
				...
				if (connectionStyle.useDataDefinedColors())
				{
					QColor const c = connectionStyle.normalColor(dataType.color);
					painter->setPen(c);
					painter->setBrush(c);
				}
				else
	...
	QPainter* painter, NodeGeometry const& geom, NodeDataModel const* model, NodeGraphicsObject const& graphicsObject)
{
	...
	if (modelValidationState != NodeValidationState::Valid)
	{
		...
		QRectF boundary(
			-diam, -diam + geom.height() - geom.validationHeight(), 2.0 * diam + geom.width(), 2.0 * diam + geom.validationHeight());
